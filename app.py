...
def send_code():
    ...
    conn = mysql.connector.connect(host='localhost', user='root', password='Cjj213513', database='pa') # 修改为自己的数据库连接信息
    cursor = conn.cursor()
...
def login_email():
    ...
    code = data['code']
    # 修改为自己的数据库连接信息
    conn = mysql.connector.connect(host='localhost', user='root', password='Cjj213513', database='pa')
    cursor = conn.cursor()
